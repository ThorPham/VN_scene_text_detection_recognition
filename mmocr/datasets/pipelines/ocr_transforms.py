...
from straug.blur import  DefocusBlur, MotionBlur, GlassBlur, ZoomBlur
...
class MyAuggemt:
    ...
    def __init__(self,p=0.5, **kwargs):
        self.type_augment = [GaussianNoise, ShotNoise,ImpulseNoise	,SpeckleNoise,
        DefocusBlur, MotionBlur, GlassBlur, ZoomBlur,Fog, Snow, Frost, Rain, Shadow,
    ...
    def __call__(self, results):
        ...
        #     img = results['img']
        #     img = Image.fromarray(img)
        #     aug = random.choice(self.type_augment)
        #     try : 
        #         img = aug()(img)
        #         img = np.array(img)
        #     except :
        #         print('error augment',aug)
        #     results['img'] = img
        if  random.uniform(0, 1) > 0.5:
            im = results['img']
            im = cv2.resize(im, (160, 48))
