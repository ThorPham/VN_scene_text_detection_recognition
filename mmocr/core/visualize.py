...
def processing_point(point,shape):
    if point < 0 :
        return 0
    if point > shape :
        return shape
...
def draw_texts_by_pil(img, texts, boxes=None, draw_box=True, on_ori_img=False,save=None):
    ...
    for idx, (box, text) in enumerate(zip(boxes, texts)):
        if len(text) == 0:
            continue
        # tmp = box.copy()
        # new_box = [[processing_point(x,h), processing_point(y,w)] for x, y in zip(box[0::2], box[1::2])]
        # new_box = np.array(new_box).flatten().tolist()
        # new_box.append(text.replace('<UKN>',''))
        ...
        font_path = os.path.join('/home/thorpham/Documents/challenge/mmocr/font/tahomabd.ttf')
...
def det_recog_show_result(img, end2end_res, out_file=None,save_txt=None):
    ...
    if is_contain_chinese(''.join(texts)):
        ...
        text_vis_img = draw_texts_by_pil(img, texts, boxes,save=save_txt)
